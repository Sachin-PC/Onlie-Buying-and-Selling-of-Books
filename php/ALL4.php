...
include_once 'sample.php';
 ...
 $res=mysqli_query($conn,"SELECT * FROM `seller` WHERE SEM=4");
  $sss=mysqli_query($conn,"SELECT * FROM `seller` WHERE SEM=4");
  $sels=mysqli_fetch_array($sss);
 $bbb=mysqli_query($conn,"SELECT * FROM `buyer` WHERE SEM=4");
...
<?php
$res=mysqli_query($conn,"SELECT * FROM `buyer`WHERE SEM=4");
print"<hr>";
while($arr=mysqli_fetch_array($res))
{
	print"<strong>BookName:</strong>{$arr['Bookname']}<br>";
