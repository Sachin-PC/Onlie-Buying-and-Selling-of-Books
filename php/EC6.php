...
include_once 'sample.php';
 ...
 $res=mysqli_query($conn,"SELECT * FROM `seller` WHERE Branch='Electronics and Communications' AND SEM=6");
  $sss=mysqli_query($conn,"SELECT * FROM `seller` WHERE Branch='Electronics and Communications' AND SEM=6");
  $sels=mysqli_fetch_array($sss);
 $bbb=mysqli_query($conn,"SELECT * FROM `buyer`WHERE Branch='Electronics and Communications'AND SEM=7");
...
<?php
$res=mysqli_query($conn,"SELECT * FROM `buyer`WHERE Branch='Electronics and Communications'AND SEM=6");
print"<hr>";
while($arr=mysqli_fetch_array($res))
{
	print"<strong>BookName:</strong>{$arr['Bookname']}<br>";
