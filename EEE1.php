...
include_once 'sample.php';
 ...
 $res=mysqli_query($conn,"SELECT * FROM `seller` WHERE Branch='Electrical Science' AND SEM=1");
?>
<html>
<head>
...
<?php
$res=mysqli_query($conn,"SELECT * FROM `buyer`WHERE Branch='Electrical Science'AND SEM=1");
print"<hr>";
while($arr=mysqli_fetch_array($res))
{
	print"<strong>BookName:</strong>{$arr['Bookname']}<br>";
