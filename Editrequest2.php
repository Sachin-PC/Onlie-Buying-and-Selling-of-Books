...
if(isset($_POST["btn-signup"]))
{
 ...
 $expectprice =$_POST['Expectedp'];
  $d=mysqli_query($conn,"SELECT Mobno FROM `user` WHERE UID='$usid'");
		$r=mysqli_fetch_array($d);
		$mobno=$r['Mobno'];
 if(mysqli_query($conn,"UPDATE `buyer` SET Bookname='$Bname',Author='$author',Branch='$branch',Sem='$sem',Edition='$edition',Publishdate='$publisheddate',Mobno='$mobno',marketp='$mrp',Expectedprice='$expectprice' WHERE ReqID='$ptno' AND UID='$id'"))
 {
  ?>
...
$pdt=$_SESSION["p"];   //Publisheddate
 $mktprc=$_SESSION["mprc"]; //MARKET PRICE
  $expprc=$_SESSION["eprc"];//SELLING PRICE
 ?>
<tr>
...
<tr>
<td>Expected Price: <input type="text" name="Expectedp" value=<?php echo"$expprc"; ?> required /></td>
</tr>
<tr>
<td><button type="submit" name="btn-signup">Edit</button></td>
</tr>
</table>
</form>
</div>
